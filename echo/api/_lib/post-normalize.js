...
export function eventSlug(title, dateIso) {
  ...
  return `echo-${translitSlug(title, 28)}${d ? '-' + d : ''}`;
}
...
export function normalizeAnnouncement(extracted, ctx) {
  ...
  const title = String(ev.title || '').trim().toUpperCase().slice(0, 60) || 'ECHO PARTY';
  if (!ev.title) problems.push('название не распознано — подставлено «ECHO PARTY»');
  ...
  const timeEnd = /^\d{2}:\d{2}$/.test(String(ev.timeEnd || '')) ? ev.timeEnd : '21:00';
  ...
  let waves = Array.isArray(ev.prices)
    ? ev.prices
        ...
        .filter((p) => {
          const okPrice = p.priceRub === 0 || (p.priceRub >= 50 && p.priceRub <= 5000);
  ...
  if (!waves.length) {
    waves = DEFAULT_WAVES;
    problems.push('цены не распознаны — подставлен стандартный прайс 500/600');
  }
