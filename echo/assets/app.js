...
function renderHero() {
  ...
  sticky.href = `/e/${e.id}`;
  sticky.textContent = price === null ? 'Смотреть афишу' : price === 0 ? 'Фри-проходка на ночную' : `Билеты от ${price} ₽`;
...
function renderMarquee() {
  const words = [
    'DAY 14+', '★', 'NIGHT 18+', '★', 'RED CUPS', '★', 'МОНТАЖНИКОВ 2/1', '★',
    'ВХОД ПО QR', '★', 'ФРИ ДО 00:30', '★', 'ОРЕНБУРГ', '★',
  ];
  const half = words
    ...
    .map((w) => {
      ...
      return `
        <div class="wave-row is-${w.state}">
          ...
          <div></div>
          <div class="wave-price">${w.priceRub === 0 ? 'фри' : `${w.priceRub} ₽`}</div>
...
function cardHtml(e) {
  ...
  else if (price === 0) priceHtml = `<small>от</small> фри`;
...
function initCityForm() {
  ...
  form.onsubmit = async (ev) => {
    ...
    if (ok) {
      ...
      const text = `Привет! Привезите ECHO в ${city}. Мой контакт: ${contact}`;
