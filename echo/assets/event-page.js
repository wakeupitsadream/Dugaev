...
function renderEvent() {
  const e = store.event;
  document.title = `${e.title} — билеты · ECHO`;
    ...
    .map((w) => {
      ...
      return `
        <div class="wave-row is-${w.state}">
          ...
          <div></div>
          <div class="wave-price">${rub(w.priceRub)}</div>
...
const rub = (v) => (v === 0 ? 'фри' : `${v} ₽`);

function updateTotal() {
  const w = store.wave;
  const total = w ? w.priceRub * store.qty : null;
  $('ot-label').textContent = `${store.qty} ${plural(store.qty, 'билет', 'билета', 'билетов')}${w ? ` · ${w.name.toLowerCase()}` : ''}`;
  $('ot-sum').textContent = w ? rub(total) : '— ₽';
  $('submit-order').textContent = !w
    ? 'Билетов нет'
    : total === 0
      ? 'Получить проходку · фри'
      : SITE.paymentDemo
        ? `Получить билеты · ${total} ₽ (демо)`
        : `Оплатить ${total} ₽`;
  // фри-проходка — это регистрация, плашка про оплату не нужна
  const demoNote = $('demo-pay-note');
  if (demoNote) demoNote.classList.toggle('hidden', !(SITE.paymentDemo && total !== 0));
...
function showSuccess(j) {
  ...
  const noteEl = document.querySelector('#pane-success .success-note');
  if (j.amount_rub === 0) {
    noteEl.textContent = 'Фри-проходка оформлена. Каждому гостю — свой именной QR: открой билет и отправь его владельцу. На входе будь до 00:30.';
  } else if (SITE.paymentDemo) {
    noteEl.textContent = 'Демо-покупка прошла (деньги не списывались). Каждому гостю — свой именной QR: открой билет и отправь его владельцу.';
  }
