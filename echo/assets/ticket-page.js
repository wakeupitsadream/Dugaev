...
function render(parsed, t) {
  ...
  $('t-meta').innerHTML =
    ...
    // у ECHO площадка = адрес: если адрес уже содержит venue, не дублируем
...
function bindActions(parsed, t) {
  ...
  if (navigator.share) {
    ...
    share.onclick = () => {
      navigator.share({
        title: 'Твой билет — ECHO',
        text: t ? `Билет на ${t.event.title} для ${t.holderName}` : 'Твой билет ECHO',
        url: location.href,
      }).catch(() => {});
    };
  }
