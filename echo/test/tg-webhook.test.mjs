...
test('кнопка «Опубликовать» переводит черновик в onsale', async () => {
  const slug = 'echo-novaya-era-0912';
  const r = await handleUpdate(
...
test('повторная публикация — noop', async () => {
  const r = await handleUpdate(
    { callback_query: { id: 'cb2', data: 'pub:echo-novaya-era-0912', from: { id: 1 } } },
    deps(null)
  );
...
test('отмена: событие снимается с продажи', async () => {
  const r = await handleUpdate(
    { callback_query: { id: 'cb5', data: 'cancel:echo-novaya-era-0912', from: { id: 1 } } },
    deps(null)
  );
  assert.equal(r.done, 'cancelled');
  const ev = (await pg.query(`SELECT status FROM events WHERE id = 'echo-novaya-era-0912'`)).rows[0];
