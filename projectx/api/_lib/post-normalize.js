...
export function eventSlug(title, dateIso) {
  ...
  return `px-${translitSlug(title, 28)}${d ? '-' + d : ''}`;
}
...
// Возрастные рейтинги бренда — как в админской форме (api/_lib/event-form.js)
const AGE_ALLOWED = [16, 18];
...
export function normalizeAnnouncement(extracted, ctx) {
  ...
  const title = String(ev.title || '').trim().toUpperCase().slice(0, 60) || 'PROJECT X PARTY';
  if (!ev.title) problems.push('название не распознано — подставлено «PROJECT X PARTY»');
  ...
  const timeStart = /^\d{2}:\d{2}$/.test(String(ev.timeStart || '')) ? ev.timeStart : '23:00';
  const timeEnd = /^\d{2}:\d{2}$/.test(String(ev.timeEnd || '')) ? ev.timeEnd : '06:00';
  ...
  const ageRating = Number.isInteger(ev.ageRating) && AGE_ALLOWED.includes(ev.ageRating) ? ev.ageRating : 18;
  if (!Number.isInteger(ev.ageRating)) problems.push('возраст не распознан — подставлен 18+');
  else if (!AGE_ALLOWED.includes(ev.ageRating)) problems.push(`возраст ${ev.ageRating}+ не для этого бренда — подставлен 18+`);
  ...
  if (!waves.length) {
    waves = DEFAULT_WAVES;
    problems.push('цены не распознаны — подставлен стандартный прайс 400/600/800');
  }
