...
async function init() {
  ...
  initFaceControl();
  initCityForm();
  $('rules-contact').href = SITE.instagramDm;
...
function renderWavesPanel() {
  ...
  $('wp-rows').innerHTML = ws
    .map((w) => {
      ...
      else if (w.state === 'active') leftNote = `<b>${w.left}</b> ${plural(w.left, 'проходка', 'проходки', 'проходок')} по этой цене`;
  ...
  const buy = $('wp-buy');
  buy.href = `/e/${e.id}`;
  buy.textContent = active ? `Успеть за ${active.priceRub} ₽` : 'Все проходки проданы';
...
function renderAfisha() {
  ...
  if (!filtered.length) {
    grid.innerHTML = `<p class="muted">В этом городе пока ничего не анонсировано — следи за <a class="acid" href="${esc(SITE.instagram)}" target="_blank" rel="noopener">Instagram</a>.</p>`;
    return;
  }
    ...
    <a class="event-card ${isPast ? 'is-past' : ''}" href="/e/${esc(e.id)}">
      ...
      <div class="ec-body">
        ...
        <div class="ec-foot">
          ...
          <span class="ec-go">${isPast ? 'фотоотчёт' : 'проходки →'}</span>
        </div>
      </div>
    </a>`;
}
